...
Smoke 測試涵蓋首頁、搜尋、聖地列表導覽、手機「更多」導覽 sheet。CI 於 `main` 與 PR 皆執行 E2E。
...
- **可靠性**：首頁 DB 不可用時顯示降級 banner（非靜默全空）；anime/profile/travelogue/routes 段錯誤頁可重試
- **效能**：公開 profile 摘要與 Wrapped 頁有 60s/120s `cacheFetch`；Wrapped `topAnime` 單次批次查詢；搜尋 anime 同步改批次 upsert
- **Loading**：Wrapped、新增/編輯聖地有 loading 殼層
- **CI**：E2E 在 PR 上亦會阻擋 merge；空結果 smoke 改測聖地分頁（避免 AniList 超時）

### Wave 27 驗收要點

- **i18n**：搜尋/通知/路線/聖地列表 UI 支援日文；通知類型標籤來自 messages
- **通知**：`createNotification` 統一觸發 Push；`preferred_locale` 寫入 DB；新通知依收件者語系模板
- **可靠性**：spots/visits/lists/feed/bookmarks 段錯誤頁可重試
- **Bundle**：JapanMap、路線規劃器、清單編輯器 dynamic import；作品討論用 LazyCommentThread
- **快取**：排行榜/公開遊記/公開路線 `revalidate = 60`
- **測試**：notification-copy + cache vitest；E2E 開發登入與日文搜尋標題 smoke

### Wave 28 驗收要點

- **修復**：清單詳情頁可正常載入編輯器；作品頁「儲存評價」無需先改狀態即可寫入 score/review/tags
- **作品瀏覽**：`/anime` 支援本季新番、Trending、有聖地作品三個分頁與類型篩選；導航列有「作品」入口
- **個人統計**：Profile overview 顯示巡禮狀態分佈、平均評分、偏好類型、建議巡禮天數
- **清單/短評**：作品頁可加入清單；作品頁「巡禮心得」與 Profile 作品分頁顯示短評
- **測試**：anime-browse 與 PATCH 邏輯 vitest；E2E 含 `/anime`、清單詳情、評分儲存 smoke
